...
fn run_sequential_step(
    ...
    if runtime_inputs
        .get("clk")
        .and_then(LogicValue::to_bit_value_checked)
        .is_some_and(|clock| !clock.is_zero())
    {
        let mut low_inputs = runtime_inputs.clone();
        low_inputs.insert("clk".into(), LogicValue::zero(1));
        sim.step(low_inputs)?;
    }
...
fn compare_outputs(
    actual: &BTreeMap<String, LogicValue>,
    expected: &BTreeMap<String, LogicPattern>,
    step: Option<usize>,
) -> Vec<JsonTestFailure> {
    let mut failures = Vec::new();
    ...
    for signal in signals {
        let value = if let Some(value) = actual.get(signal).cloned() {
            ...
            let instance_path = path
                ...
                .collect::<Vec<_>>();
            sim.read_signal(&runtime_inputs, &instance_path, signal_name)?
        } else {
            return Err(Error::Resolve(format!(
...
mod tests {
    ...
    use crate::{Compiler, LogicPattern, LogicValue};
    ...
    #[test]
    fn run_json_file_passes_four_state_public_boundary_suite() {
        let repo = repo_root();
        let design = Compiler::new()
            .compile_file(repo.join("parts/testing/023-FourStateBoundary.sv"))
            .expect("compile four-state boundary");

        let report = design
            .run_json_file(repo.join("parts/testing/023-FourStateBoundary.json"))
            .expect("run json tests");
    ...
    #[test]
    fn run_json_file_passes_picorv32_jump_link_suite() {
        let repo = repo_root();
        let design = Compiler::new()
            .add_search_path(repo.join("parts/picorv32"))
            .compile_file(repo.join("parts/picorv32/picorv32_program_harness.sv"))
        ...
        let report = design
            .run_json_file(repo.join("parts/picorv32/demo_jump_link.json"))
            .expect("run picorv32 jump-link json");

        assert!(
            report.all_passed(),
            "picorv32 jump-link report:\n{}",
            serde_json::to_string_pretty(&report).expect("serialize report")
        );
        assert_eq!(report.passed, report.total);
    }
    ...
    #[test]
    fn run_json_file_passes_picorv32_subword_memory_suite() {
        let repo = repo_root();
        let design = Compiler::new()
            .add_search_path(repo.join("parts/picorv32"))
            .compile_file(repo.join("parts/picorv32/picorv32_program_harness.sv"))
        ...
        let report = design
            .run_json_file(repo.join("parts/picorv32/demo_subword_mem.json"))
            .expect("run picorv32 subword-memory json");

        assert!(
            report.all_passed(),
            "picorv32 subword-memory report:\n{}",
            serde_json::to_string_pretty(&report).expect("serialize report")
        );
        assert_eq!(report.passed, report.total);
    }
    ...
    #[test]
    fn run_json_file_accepts_four_state_inputs_after_runtime_cutover() {
