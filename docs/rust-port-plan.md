...
- measured `parts/picorv32` status is now `3/3` clean compile-only source files (`picorv32.v`, `picorv32_program_harness.sv`, `picorv32_smoke.sv`) and `10/10` passing JSON suites
- remaining work is expanding PicoRV32 execution beyond the current checked-in straight-line, taken-branch, signed/unsigned-compare, jump/link, load-backed, subword-memory, multi-store, and smoke-harness programs; the next runtime gap is explicit misaligned-access and trap behavior even though the HIR is now compile-clean
