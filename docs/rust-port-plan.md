...
- measured `parts/picorv32` status is now `3/3` clean compile-only source files (`picorv32.v`, `picorv32_program_harness.sv`, `picorv32_smoke.sv`) and `5/5` passing JSON suites
- remaining work is expanding PicoRV32 execution beyond the current checked-in straight-line, multi-store, and smoke-harness programs; taken-branch control flow remains the next runtime gap even though the HIR is now compile-clean
