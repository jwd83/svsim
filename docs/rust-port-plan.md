...
- measured verification: `cargo test` passes; the compile-only multi-directory corpus reports `parts/basic` at `44/44`, `parts/testing` at `40/40`, `parts/overture` at `41/41`, and the full `parts/basic` + `parts/testing` + `parts/overture` compile surface at `125/125` in about `1.9s`; the JSON regression corpus remains green at `127/127` in about `18.4s`
- measured batch status: the compile-only multi-directory runner completes `parts/basic` in about `0.9s`, `parts/testing` in about `0.2s`, and `parts/overture` in about `0.8s`; the JSON multi-directory runner completes them in about `7.7s`, `0.2s`, and `10.5s`
...
- duplicate module definitions, duplicate declarations, duplicate instance names, identifier/memory resolution, lowered select bounds, unsupported `inout` / `ref` ports, input-port drive attempts, and the most obvious invalid named-port bindings, including missing child input connections, are now rejected before simulation
- width normalization and moving more runtime-only value-width checks into a single elaboration stage are still future work
