...
- duplicate module definitions, duplicate declarations, duplicate instance names, identifier/memory resolution, lowered select bounds, unsupported `inout` / `ref` ports, input-port drive attempts, the most obvious invalid named-port bindings including missing child input connections, and the current 64-bit runtime width limit for declarations, literals, concatenations, replications, and concatenated lvalues are now rejected before simulation
...
- measured Overture status now includes `41/41` clean compile-only source files and `43/43` passing JSON suites, including the two explicit-source `overture_cpu` program variants; the full multi-directory corpus snapshot is `125/125` compile-only in about `2.4s` and `127/127` JSON regressions in about `19.1s`
- remaining work is folding more in-range resolution checks into a single elaboration/validation layer, especially width normalization within the supported `1..=64` bit subset, and tightening unsupported-construct diagnostics where wider coverage finds gaps
...
That library milestone, the compile-only and JSON corpus runners, the first compile-time semantic validation pass, and the current fixpoint engine are now in place. The next pragmatic target is to keep pulling in-range resolution checks into a single elaboration layer, with width normalization inside the supported `1..=64` bit subset now the clearest remaining slice, then continue the runtime shift away from iterative hot paths toward a more explicit compiled evaluation model.
