...
Current implementation status as of March 20, 2026:
...
- library-side JSON regression execution is now available through `CompiledDesign::run_json_file` / `svsim::JsonTestSuite` for combinational arrays, sequential `test_cases`, relative memory-file preload, and per-step traces of both top-level outputs and hierarchical `instance.signal` names
...
- `SimulationSession::read_signal` now exposes settled signal reads by instance path, so embedding callers and JSON regressions can inspect internal control-path state without adding temporary debug ports
- constant-condition pruning now short-circuits lowered `&&` / `||` during parameter evaluation, so dead feature-gated PicoRV32 branches stay out of the executable subset without reintroducing the runtime comparison/logical precedence bug uncovered by `mem_wordsize == 0 && reg_op1[1:0] != 0`
- unsized decimal integer literals now lower as 32-bit values, so Verilog masks like `& ~1` preserve their full-width semantics instead of collapsing PicoRV32 branch targets to zero
...
- measured `parts/picorv32` status is now `3/3` clean compile-only source files (`picorv32.v`, `picorv32_program_harness.sv`, `picorv32_smoke.sv`) and `13/13` passing JSON suites
- the PicoRV32 harness now stages visible RAM writes for one cycle before committing them, which keeps trap-on-store cases from mutating the harness RAM window after the core has already decided to trap
- the PicoRV32 executable corpus now also includes a misaligned-`jalr` instruction-path trap case, proving the rewrite enters trap state before any visible store commits when `reg_pc` reaches `0x0000_0006`
- remaining work is expanding PicoRV32 execution beyond the current checked-in straight-line, taken-branch, signed/unsigned-compare, jump/link, instruction-misaligned-`jalr`, load-backed, subword-memory, misaligned-load/store, multi-store, and smoke-harness programs; the next runtime gap is the remaining taken-branch instruction-misalignment case even though the HIR is now compile-clean
