...
- compilation now includes a first semantic validation pass over lowered HIR, catching duplicate declarations, undeclared identifiers/memories, lowered select bounds, and invalid named-port instance bindings before simulation
...
- measured verification: `cargo test` passes; the compile-only multi-directory corpus reports `parts/basic` at `44/44`, `parts/testing` at `40/40`, `parts/overture` at `41/41`, and the full `parts/basic` + `parts/testing` + `parts/overture` compile surface at `125/125` in about `1.9s`; the JSON regression corpus remains green at `127/127` in about `16.7s`
- measured batch status: the compile-only multi-directory runner completes `parts/basic` in about `0.9s`, `parts/testing` in about `0.2s`, and `parts/overture` in about `0.8s`; the JSON multi-directory runner completes them in about `7.7s`, `0.2s`, and `8.8s`
...
- duplicate declarations, identifier/memory resolution, lowered select bounds, and the most obvious invalid named-port bindings are now rejected before simulation
- width normalization, richer port binding rules, and moving more runtime-only checks into a single elaboration stage are still future work
...
- compilation now rejects several design-shape errors before simulation, including undeclared identifiers, duplicate declarations, unknown named ports, duplicate named-port bindings, and non-lvalue output bindings
...
- measured Overture status now includes `41/41` clean compile-only source files and `43/43` passing JSON suites, including the two explicit-source `overture_cpu` program variants; the full multi-directory corpus snapshot is `125/125` compile-only in about `1.9s` and `127/127` JSON regressions in about `16.7s`
...
That library milestone, the compile-only and JSON corpus runners, the first compile-time semantic validation pass, and the current fixpoint engine are now in place. The next pragmatic target is to keep pulling runtime-only resolution checks into a single elaboration layer, then continue the runtime shift away from iterative hot paths toward a more explicit compiled evaluation model.
